...
export type LookupFn = (hostname: string, options: { all: true; verbatim: true }) => Promise<LookupResult[]>;
...
const defaultDnsLookupFn: LookupFn = async (hostname, options) => {
  const result = await lookup(hostname, options);
  return result as LookupResult[];
};

export class UrlGuardService {
  constructor(private readonly maxUrlLength: number, private readonly dnsLookupFn: LookupFn = defaultDnsLookupFn) {}
