...
export function decodeHtmlEntities(value) {
    return value.replace(/&(#x[0-9a-f]+|#\d+|[a-z]+);/gi, (fullMatch, entity) => {
...
function collapseDuplicateParentheses(value) {
    let text = value;
...
export function normalizeDescription(description) {
    ...
    if (typeof description === "object") {
        if (description.text)
            return normalizeDescription(description.text);
        if (description.name)
            return normalizeDescription(description.name);
    }
...
export function normalizeCategory(category) {
    ...
    if (typeof category === "object") {
        if (category.name)
            return sanitizeText(category.name);
        if (category.text)
            return sanitizeText(category.text);
    }
    return "";
}
    ...
    if (Array.isArray(ingredients)) {
        return ingredients
            ...
            if (typeof ingredient === "string")
                return sanitizeText(ingredient);
            if (ingredient && ingredient.text)
                return sanitizeText(ingredient.text);
            return "";
        })
    ...
    if (typeof time === "object") {
        if (time.duration || time.value) {
            ...
            if (typeof duration === "string")
                return normalizeTimeString(duration);
        }
        if (time.minutes || time.hours) {
            let totalMinutes = 0;
            if (time.hours)
                totalMinutes += parseInt(time.hours, 10) * 60;
            if (time.minutes)
                totalMinutes += parseInt(time.minutes, 10);
            return totalMinutes.toString();
        }
    }
...
export function normalizeServingSize(servings) {
    if (!servings)
        return "";
    ...
    if (Array.isArray(servings))
        return sanitizeText(servings[0] || "");
    return sanitizeText(servings);
}
...
export function findRecipeObject(obj) {
...
export function extractJsonLdBlocks(html) {
    ...
    while ((match = scriptRegex.exec(html)) !== null) {
        ...
        if (!raw || raw.length > MAX_JSON_LD_SCRIPT_BYTES)
            continue;
        blocks.push(raw);
    }
    return blocks;
}
export function buildRecipeFromSchemaData(recipeData, source) {
...
export class RecipeParserService {
    ...
    parseWithDiagnostics(html, source) {
        ...
        if (instructionEls.length === 1) {
            ...
            if (el.find("li").length > 0) {
                ...
                const text = el.text().trim();
                if (text)
                    instructions = [sanitizeText(text)];
            }
        }
        else {
...
//# sourceMappingURL=recipeParserService.js.map