...
function isTransientAxiosError(error) {
    ...
    if (status && [429, 502, 503, 504].includes(status))
        return true;
    return ["ECONNABORTED", "ECONNRESET", "EAI_AGAIN", "ETIMEDOUT", "ENOTFOUND"].includes(String(error.code));
}
function isBlockLikeAxiosError(error) {
    if (!axios.isAxiosError(error))
        return false;
    const status = error.response?.status;
...
export class HtmlFetchService {
    constructor(httpGetter, urlGuard, fallbackHttpGetter) {
        this.httpGetter = httpGetter;
        this.urlGuard = urlGuard;
        this.fallbackHttpGetter = fallbackHttpGetter;
    }
    async fetchHtmlWithRetry(url) {
        const result = await this.fetchHtmlWithRetryDetailed(url);
        return result.response;
    }
    async fetchHtmlWithRetryDetailed(url) {
    ...
    async fetchHtmlSafely(initialUrl) {
        let currentUrl = initialUrl;
        let fetchProfile = "primary";
...
//# sourceMappingURL=htmlFetchService.js.map